...
description: 小红书公开/半公开内容采集与本地化分析 skill。能力：关键词搜索笔记、单篇笔记详情 + 评论、用户主页笔记、热门榜、用户搜索，以及 collect→clean→enrich→analyze 全链路，输出洞察式 Markdown 报告 + CSV 明细。适用：用户要求抓取/分析/导出小红书笔记、评论、用户、热门榜数据时使用。前提：用户提供本人账号的登录 cookie。约束：不绕过登录态、不并发不高频、拒绝绕过签名风控或未授权数据的抓取。调用约定：skill 输出正文 (desc_plain) + 评论文本，**由 agent 基于用户关心的具体问题动态生成针对性总结报告**，skill 不预设任何总结模板。
---
...
```
输出到 `data/runs/<日期>_<topic>/`（7 个文件，见 §5）。跑完向用户汇报：**report.md 路径 + 基于正文与用户具体问题的针对性总结要点**（不要只丢一个路径，也不要复用 §1 的通用模板结论）。
...
skill 不预设任何总结模板。`report.md` 的 §1 核心结论是通用统计骨架，不替代给用户的最终答案。Agent 拿到 report.md 后按以下步骤产出针对性总结：

1. **读 `enriched.jsonl`**（不只是 `report.md`）—— §1-§7 都是聚合统计，**真正的素材在每条记录的 `desc_plain` (笔记正文) 和 `content` (评论正文)** 里。
2. **从用户消息解析"关心什么"** —— 几种典型形态：
   - "装备怎么选" → 聚焦带 `desc_plain` 含装备/型号/价格 的笔记
   - "安全吗" → 聚焦 `keywords` / `desc_plain` 含 危险/隐患/翻车 的笔记
   - "新手踩雷" → 聚焦 `sentiment = negative` 且 `is_short = true` 的笔记
   - "适合周末两天吗" → 聚焦 `interact` 数据 + 评论疑问 Top 笔记
   - 用户没明确说什么 → 默认"综合概览"：选 Top 3 笔记正文 + Top 5 真实讨论评论，给一段连贯解读
3. **生成针对性总结** —— 不是把 §1 抄一遍，而是基于 desc_plain + content + 用户问题，写一段 200-400 字的归纳。
4. **汇报路径** + 总结要点（不要只丢路径）。

**文件读取入口**：
- 正文：`data/runs/<date>_<topic>/enriched.jsonl`，每行一条记录，字段 `desc_plain` (笔记) / `content` (评论)
- 报告：`data/runs/<date>_<topic>/report.md`（骨架，§8 / §10 是已补全正文展示）

**反面示例（不要做）**：
- ❌ 把 `report.md` 的 §1 核心结论直接转给用户（"主题集中度最高频关键词「露营」..."）
- ❌ 只贴点赞量/话题统计（用户已经反馈过这没意义）
- ❌ 只说"完整正文见 report.md"（等于没总结）

**正面示例**：
- ✅ "根据正文，新手最容易踩的 3 个坑：1) 夏天不选自动帐篷（封闭闷热）2) 不选平折式营地车 3) 不选实木蛋卷桌。评论区问得最多的是「卡式炉配不锈钢桌会不会爆炸」—— 没看到明确答案，需要继续看 Top 5 真实讨论。"
