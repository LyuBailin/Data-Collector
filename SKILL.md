...
2. **针对每个调研维度, 选 Top 笔记 + Top 评论** (聚合 JSON 里已按赞数排好)
3. **对于有 `desc_plain` 的笔记 (聚合 JSON `full_notes` 字段), 直接引用原文** — 不要改写、不要概括、不要创作
4. **对于评论, 直接引用 `content` 原文**, 标 `run` + `note_id` + `user` (匿名评论标 "匿名")
5. **每个结论至少 1 条证据**, 不允许"我认为" / "感觉上" 类无源断言
6. **如果维度数据稀薄** (聚合 JSON 里某维度命中笔记数 < 3), 在报告里明示并建议补跑关键词
...
>
> 评论区佐证: `hcsuojian/run` note `6a852bc4` @We1L (46 赞) 提到 '今年秋招体感温度非常低, 阿里云也是煎熬'..."
